...
<p className="font-semibold text-[#0b2b35] mt-4">Leave balances and accruals</p>
            <p>Every balance load — the initial Data Import and any later override — must state the date its numbers are <strong>as of</strong> (pre-filled from the file&apos;s own &ldquo;As Of Date&rdquo; cell when it has one, never assumed, never in the future); the date is stored with an audit record of who loaded what, from which file, and when. Balances are loaded from CHA&apos;s figures and can be overridden at any time under <strong>Admin Console → Leave Balances</strong> (President/CEO, Accounting Manager, and super administrator only): upload the file, set its &ldquo;as of&rdquo; date, review the before/after preview, and confirm. Approved leave dated on or after the as-of date is deducted again automatically, and every override is recorded (who, when, and each employee&apos;s before and after). PTO and sick <strong>accruals</strong> run per pay period only once they are switched on there, starting from the first period <em>after</em> the one the loaded balances already include; each period is credited once, so re-running never double-credits.</p>
            <p className="font-semibold text-[#0b2b35] mt-4">Keeping the portal and Sage in step</p>
            <p>The portal is the <strong>live ledger</strong> — leave is entered daily and balances move as it happens — while Sage catches up only once it has processed a pay period. Each cycle: (1) staff record leave as it happens and approvers clear anything pending; (2) timesheets are submitted and approved; (3) accounting closes the period under <strong>Close Period</strong>, which saves every employee&apos;s balances as of the last closed date; (4) once Sage has processed the same period, upload Sage&apos;s balances under <strong>Admin Console → Leave Balances → Reconcile with Sage</strong>. The comparison lines them up against the portal&apos;s balances <em>as of the same date</em> and lists only the differences; nothing is overwritten. A difference is investigated first, then either corrected in the system that is wrong or posted as a documented adjustment (effective date and reason recorded). Bulk overrides from a file are for the initial load and rare corrections, not for routine syncing, because they erase live activity. Approvers receive a weekday digest of anything that has waited on them more than 2 days, since slow approvals are what let the two systems drift apart.</p>
          ...
          <Section id="security" title="9. Account Security">
            <Bullets items={[
              ...
              'Salary information and the team Timesheets report are restricted to the President/CEO, the Accounting Manager, and the system super administrator only (not to other admins); every other employee sees only their own timesheets, history, and gross wages. Salary amounts are masked by default and each must be clicked individually to be shown; a revealed amount hides itself again after a few seconds, so do not leave salary figures on screen or share your screen while one is showing. Credentials are personal and must not be shared; each employee is individually responsible for the accuracy of entries and e-signatures made under their own login.',
            ]} />
          </Section>
