...
export default async function ReportsPage() {
  ...
  const totalPtoUsed = rows.reduce((s, r) => s + r.pto_used, 0)
  const totalSickUsed = rows.reduce((s, r) => s + r.sick_used, 0)
    ...
    <div className="print:p-0">
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-[13px] text-gray-500 mt-0.5">
            {isManager ? 'All staff — Community Housing Associates' : `My leave summary — ${currentUserName}`}
          </p>
        </div>
        <ExportPdfButton />
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          { label: isManager ? 'Employees Reported' : 'Leave Requests', value: rows.length, color: 'text-[#0b2b35]' },
        ].map(s => (
        ...
        <table className="w-full text-[13px]">
          <thead>
            <tr className="bg-[#f9fefe] border-b border-[#d4eef2]">
              {(isManager
                ? ['Employee', 'PTO Used', 'Sick Used', 'Personal Used', 'PTO Balance', 'Sick Balance']
                : ['PTO Used', 'Sick Used', 'Personal Used', 'PTO Balance', 'Sick Balance']
              ).map(h => (
          ...
          <tbody>
            {rows.map(r => (
              <tr key={r.name} className="border-b border-[#f0f7f8] last:border-0">
                {isManager && <td className="px-5 py-3 font-medium">{r.name}</td>}
