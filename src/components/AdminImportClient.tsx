...
import { parseEmployeeFile, parseBalanceFile, parseSalaryFile, validateImport, commitImport, inviteEmployees } from '@/app/actions/import'
...
export default function AdminImportClient({ isSuperAdmin, superAdminName }: { isSuperAdmin: boolean; superAdminName: string | null }) {
  ...
  async function handleCommit() {
    ...
    try {
      ...
      const res = await commitImport({ employees: okEmployees, balances: okBalances, salaries: okSalaries })
      setResult(res)
      setStep('done')
    } catch (e: unknown) {
            ...
            <div>
              {canContinue ? (
                <div className="bg-[#f8fcfd] border border-[#d4eef2] text-[#0b2b35] text-[13px] rounded-lg px-4 py-3 mb-3">
                  This batch is ready.{superAdminName ? ` ${superAdminName}` : ' The system admin'} needs to review and complete the import — let them know it&apos;s ready to commit.
                </div>
              ) : (
                <div className="bg-red-50 border border-red-200 text-red-600 text-[13px] rounded-lg px-4 py-3 mb-3">
                  Resolve the error(s) above before this can be reviewed for import.
                </div>
              )}
              <button onClick={reset} className="border border-[#d4eef2] text-[13px] font-semibold px-5 py-2 rounded-lg hover:bg-[#f0f7f8]">
                Start Over
